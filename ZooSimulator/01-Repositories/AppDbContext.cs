...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        In Entity Framework Core (EF Core), a discriminator is a concept used in Table-per-Hierarchy (TPH) inheritance to distinguish between different types of entities stored in the same table. Here's an overview of what a discriminator is and how it works:

        What is a Discriminator?
        Purpose: The discriminator is a column in the table that EF Core uses to determine the specific subclass of an entity when querying data. It helps EF Core understand which type of entity (e.g., Lion, Eagle, Tiger) each row in the table represents.
        Function: When querying the database, EF Core uses the discriminator column to filter and instantiate the correct subclass of the base class (Animal in your example).
        */
